...
class CustomCooldown(commands.Cog):
    ...
    def __init__(self, bot : Red):
        ...
        default_guild = {"cooldown_channels" : {}, "cooldown_categories" : {}, "allowed_users" : [], "send_dm" : False}
    ...
    async def handle_channel_cooldown(self, message, cooldown_channels, send_dm):
        ...
        if str(user.id) not in channel_data["users_on_cooldown"]:
            channel_data["users_on_cooldown"][str(message.author.id)] = now.strftime(self.date_format)
            ...
            if total_seconds <= channel_data["cooldown_time"]:
                ...
                if send_dm:
                    try:
                        ...
                        await user.send(f"Slow down! You'll be able to post again in {channel.mention} in {remaining_time}")
            ...
            if total_seconds <= category_data["cooldown_time"]:
                    ...
                    if send_dm:
                        try:
                            ...
                            remaining_time = chat_formatter.humanize_timedelta(seconds=s)
                            await user.send(f"Slow down! You'll be able to post again in {channel.mention} in {remaining_time}. Please note that this channel shares a rate limit with it's parent channel category.")
                        except discord.Forbidden as e:
                            pass
            else:
                category_data["users_on_cooldown"][str(user.id)] = now.strftime(self.date_format)
    ...
    def is_mod_or_allowed():
        async def predicate(ctx):
            bot = ctx.bot
            config = ctx.cog.config
            is_mod = await bot.is_mod(ctx.author)
            allowed_users = await config.guild(ctx.guild).allowed_users()
            is_allowed = ctx.author.id in allowed_users
            if is_mod or is_allowed:
                return True
    ...
    async def category(self, ctx, category : discord.CategoryChannel, time : str):
        ...
        if str(category.id) in cooldown_categories:
            await ctx.send("The specified category is already on cooldown do you want to edit it's time ? [yes/no]")
            ...
            cooldown_categories[str(category.id)] = {"cooldown_time" : cooldown_time, "users_on_cooldown" : {}, "channels" : [channel.id for channel in category.channels]}
            await self.config.guild(ctx.guild).cooldown_categories.set(cooldown_categories)
            await ctx.send(f"{category.name}'s channels are now set at 1 message every {time}")
    ...
    async def channel(self, ctx, channel : discord.TextChannel, time : str):
        ...
        if str(channel.id) in cooldown_channels:
            await ctx.send("The specified channel is already on cooldown do you want to edit it's time ? [yes/no]")
            pred = MessagePredicate.yes_or_no(ctx)
            try:
                await ctx.bot.wait_for("message", check=pred, timeout=30)
            except asyncio.TimeoutError:
            ...
            if pred.result is True:
                ...
                return await ctx.send(f"No changes were made to {channel.mention} cooldown settings.")
        else:
            if cooldown_time == 0:
                return await ctx.send("The cooldown time cannot be 0")
    ...
    @commands.group()
    async def snailset(self, ctx):
        """
        Config settings for snailset cog.
        """
        pass
    ...
    @snailset.command()
    @checks.admin()
    async def allowuser(self, ctx, user : discord.Member):
        """
        Allows a user to use the snail command
        """
        ...
        await ctx.send(f"{user} can now run `!snail`")
    ...
    @snailset.command()
    @checks.admin()
    async def disallowuser(self, ctx, user : discord.Member):
        """
        Removes access to a user that has access to snail command
        """
        allowed_users = await self.config.guild(ctx.guild).allowed_users()
        if user.id in allowed_users:
            allowed_users.append(user.id)
        await self.config.guild(ctx.guild).allowed_users.set(allowed_users)
        await ctx.send(f"{user} cannot run `!snail`")
    
    @snailset.command()
    @checks.admin()
    async def senddm(self, ctx, option : bool = True):
        """
    ...
    async def bypass_category(self, ctx, member : discord.Member, category : discord.CategoryChannel):
        """
    ...
    async def on_message(self, message: discord.Message):
        ...
        catgories_channels = []
        for category_id, category_data in cooldown_categories.items():
        ...
        if str(channel.id) in cooldown_channels:
            await self.handle_channel_cooldown(message, cooldown_channels, send_dm = send_dm)
        if channel.category and str(channel.id) in catgories_channels:
            await self.handle_category_cooldown(message, cooldown_categories, send_dm = send_dm)
